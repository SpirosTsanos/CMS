...
namespace CMS.Services.Clients
{
    class TestClientService : IClientService
    {
        ...
        public Task<Client[]> GetClientsForBranchAsync(string branchId)
        {
            return Task.Run(() =>
             {
                 Thread.Sleep(2000);
        ...
        public Task<Client> AddNewClientAsync(Client newClient)
        {
            return Task.Run(() =>
            {
                Thread.Sleep(2000);
                newClient.ClientId = Guid.NewGuid();
                _clients = _clients.Append(newClient).ToArray();
                return newClient;
            });
        ...
        public Task UpdateClientAsync(Client client)
        {
            ...
            {
                Thread.Sleep(2000);
                var idx = Array.FindIndex(_clients, c => c.ClientId.Equals(client.ClientId));
                if (idx >= 0)
                {
                    _clients[idx] = client;
                }
        ...
        public Task RemoveClientAsync(Client client)
        {
            ...
            {
                Thread.Sleep(2000);
                if (_clients.Where(c => c.ClientId.Equals(client.ClientId)).Any())
                    _clients = _clients.Where(c => c.ClientId != client.ClientId).ToArray();
            });
        }
